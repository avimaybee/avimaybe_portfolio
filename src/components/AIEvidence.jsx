...
const AIEvidence = ({ evidence }) => {
  ...
  const tabClasses = (tabName) => (
    ...
    `${activeTab === tabName ? 'bg-deep-sky-blue text-comet-white shadow-md shadow-comet-pink-peach/20' : 'text-desaturated-gray hover:text-comet-white'}`
  );
  ...
  return (
    <div className="mt-6 border-t border-desaturated-gray pt-6">
      ...
      <div className="flex space-x-2 mb-4 glass-sm p-1 rounded-full">
      ...
      <div className="py-4 glass-sm rounded-lg p-4 shadow-inner">
        {activeTab === 'beforeAfter' && (
          <div>
            {evidence.beforeAfter && evidence.beforeAfter.length > 0 ? (
              evidence.beforeAfter.map((item, index) => (
                <div key={index} className="mb-6 last:mb-0 relative group">
                  {item.description && <p className="text-comet-white text-sm mb-2">{item.description}</p>}
                  <ReactCompareSlider
            ...
            {evidence.codeGens && evidence.codeGens.length > 0 ? (
              evidence.codeGens.map((item, index) => (
                <div key={index} className="mb-6 last:mb-0">
                  <h4 className="font-semibold text-comet-white mb-2">{item.title}</h4>
